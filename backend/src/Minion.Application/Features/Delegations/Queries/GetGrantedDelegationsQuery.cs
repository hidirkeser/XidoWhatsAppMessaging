...
public record GetGrantedDelegationsQuery(DelegationFilterDto Filter) : IRequest<PaginatedList<DelegationListItemDto>>;

public class GetGrantedDelegationsQueryHandler : IRequestHandler<GetGrantedDelegationsQuery, PaginatedList<DelegationListItemDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly ICurrentUserService _currentUser;

    public GetGrantedDelegationsQueryHandler(IApplicationDbContext context, ICurrentUserService currentUser)
    {
        _context = context;
        _currentUser = currentUser;
    }

    public async Task<PaginatedList<DelegationListItemDto>> Handle(GetGrantedDelegationsQuery request, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrEmpty(f.Search))
        {
            ...
            query = query.Where(d =>
                d.DelegateUser.FirstName.ToLower().Contains(s) ||
                d.DelegateUser.LastName.ToLower().Contains(s) ||
                d.DelegateUser.PersonalNumber.Contains(s));
        }
        ...
        var projected = query
            ...
            .Select(d => new DelegationListItemDto(
                d.Id,
                d.DelegateUser.FirstName + " " + d.DelegateUser.LastName,
                d.Organization.Name,
                d.Status.ToString(),
